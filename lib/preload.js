...
const preload = (a, config) => {
  ...
  const _requestPreloadFsHash = () => {
    const modHashesJson = path.join(dataDirectory, 'mod-hashes.json');

    return Promise.all([
      _requestExists(installDirectory),
      _requestExists(modHashesJson),
    ])
      .then(([
        installDirectoryExists,
        modsHashesExists,
      ]) => {
        if (installDirectoryExists && !modsHashesExists) {
          return Promise.all([

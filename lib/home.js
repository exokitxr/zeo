  ...
  const _listenServers = () => new Promise((accept, reject) => {
    ...
    const _prepareServer = worldname => new Promise((accept, reject) => {
      ...
      mkdirp(serverDirectory, err => {
        if (!err) {
          const _requestCopy = (src, dst) => new Promise((accept, reject) => {
            fs.copy(src, dst, {
              overwrite: true,
            }, err => {
              if (!err) {
          ...
          Promise.all([
            _requestCopy(path.join(dirname, dataDirectory, 'crypto'), serverDirectory),
            _requestCopy(path.join(dirname, dataDirectory, 'installed'), serverDirectory),
          ])
            .then(accept)
            .catch(reject);
        } else {

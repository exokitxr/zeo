    ...
    .then(processes =>
      processes.map(process => {
        ...
        if (processIndexFilePath === localIndexFilePath && worldname && port) {
          ...
          const url = protocol + '://' + homeSpec.host + ':' + homeSpec.port + '/s/' + worldname;
          const directUrl = protocol + '://' + homeSpec.host + ':' + port;
      ...
      Promise.all([
        ...
        .then(([
          ...
          return Promise.all(serverSpecs.map(serverSpec => {
            ...
            if (running) {
              const directUrl = protocol + '://127.0.0.1:' + port;

              return Promise.all([
                _requestServerUsers(directUrl)
                  .catch(err => {
                    console.warn(err);

                    return Promise.resolve([]);
                  }),
                _requestServerProxyLogin(directUrl)
                  .catch(err => {
                    console.warn(err);

...
const listen = (a, config, {key, userDb}) => {
    ...
    .then(processes =>
      processes.map(process => {
        ...
        if (processIndexFilePath === localIndexFilePath && worldname && port) {
          ...
          const relativeUrl = 's/' + worldname;
          const hostUrl = protocol + '://' + homeSpec.host + ':' + port;
  ...
  const _listenServers = () => new Promise((accept, reject) => {
    const _startServer = worldname => portastic.find({
      ...
      const _requestServerProxyLogin = serverUrl => {
        const serverUrlSpec = _parseUrlSpec(serverUrl);

        if (serverUrlSpec !== null) {
          const {protocol, host, port} = serverUrlSpec;

          return fetch(`${protocol}://${host}:${port}/server/proxyLogin`, {
            method: 'POST',
          })
            .then(res => res.json()
              .then(({token}) => token)
   ...
   a.app.get('/servers/local.json', (req, res, next) => {
      Promise.all([
        new Promise((accept, reject) => {
        ...
        .then(([
          ...
          const serverSpecs = files.map(worldname => {
            ...
            const url = process ? process.relativeUrl : null;
          ...
          return Promise.all(serverSpecs.map(serverSpec => {
            ...
            if (running) {
              const hostUrl = protocol + '://127.0.0.1:' + port;

              return Promise.all([
                _requestServerUsers(hostUrl)
                  .catch(err => {
                    console.warn(err);

                    return Promise.resolve([]);
                  }),
                _requestServerProxyLogin(hostUrl)
                  .catch(err => {
                    console.warn(err);
    ...
    a.app.post('/servers/proxyLogin', bodyParserJson, (req, res, next) => {
      const {body: data} = req;
    ...
    a.app.post('/servers/create', bodyParserJson, (req, res, next) => {
      const {body: data} = req;
    ...
    a.app.post('/servers/start', bodyParserJson, (req, res, next) => {
      const {body: data} = req;
    ...
    a.app.post('/servers/stop', bodyParserJson, (req, res, next) => {
      const {body: data} = req;
  ...
  const _listenProxy = () => new Promise((accept, reject) => {
    ...
    serversProxy.on('error', err => {
      console.warn(err);
    });

    const _listenHttpProxy = () => {
      a.app.all(/^\/s\/([^\/]+)(.*)$/, (req, res, next) => {
        ...
        _requestServerProcess(worldname)
          .then(serverProcess => {
            if (serverProcess) {
              req.url = (pathname || '/') + url.format({ query: req.query });

              const {hostUrl: serverUrl} = serverProcess;

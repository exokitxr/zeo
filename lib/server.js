...
const listen = (a, config, {key, userDb}) => {
  ...
  const _prepareDirectories = () => {
    ...
    const _requestCopyDirectory = (src, dst) => {
      if (src && dst) {
        const srcDirectory = path.join(dirname, src);
        const dstDirectory = path.join(dirname, dst);

        return Promise.all([
          _requestExists(srcDirectory),
          _requestExists(dstDirectory),
        ])
          .then(([
            srcExists,
            dstExists,
            ...
            if (srcExists && !dstExists) {
              mkdirp(dst, err => {
                if (!err) {
                  fs.copy(srcDirectory, dstDirectory, {
                    overwrite: true,
                  }, err => {
                    if (!err) {
                      accept();

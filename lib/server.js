  ...
  return _ensureServerImage()
    .then(() => {
      ...
      const _listenServer = () => {
        ...
        const _requestHubLogin = token => fetch('https://' + hubUrl + '/server/login', {
          method: 'POST',
          headers: {
                ...
                if (t) {
                  ...
                  if (auth.parseToken({
                    ...
                    if (serverOfficial && hubSpec) {
                      _requestHubLogin(token)
                        .then(ok => {
                          if (ok) {
                            const newToken = base64.encode(crypto.randomBytes(RAW_TOKEN_SIZE / 8));
      ...
      const _listenServers = () => {
        ...
        a.app.get('/servers/img/icon.png', _filterIsServerHostname, (req, res, next) => {
          res.set('Access-Control-Allow-Origin', 'https://' + hubUrl);

...
const listen = (a, config, {key, userDb}) => {
  ...
  const {metadata: {site: {url: siteUrl}, hub: {url: hubUrl, enabled: hubEnabled}, dns: {url: dnsUrl, port: dnsPort, enabled: dnsEnabled}}} = config;
  ...
  const _listenServers = () => new Promise((accept, reject) => {
    ...
    class Server {
      constructor(worldname, url, users, address, addressFamily, timestamp) {
    ...
    const _listenServersRequest = () => {
      ...
      a.app.post('/servers/announce', _filterIsHubHostname, bodyParserJson, (req, res, next) => {
        ...
        if (
          ...
          const server = new Server(worldname, url, users, address, addressFamily, timestamp);
          ...
          if (!err) {
            Promise.all(processes.map(process => new Promise((accept, reject) => {
              ...
              Promise.all([
                ...
                new Promise((accept, reject) => {
                  fs.readFile(path.join('/', 'proc', pid, 'cmdline'), 'utf8', (err, cmdlineString) => {
                    if (!err) {
      ...
      a.app.post('/servers/create', _filterIsHubHostname, bodyParserJson, (req, res, next) => {
        ...
        if (typeof data === 'object' && data !== null) {
          ...
          if (typeof worldname === 'string') {
            ...
            .then(ports => {
              if (ports.length > 0) {
                const serverDirectory = path.join(dirname, dataDirectory, 'hub', 'servers', worldname);
                ...
                mkdirp(serverDirectory, err => {
                  if (!err) {
                    ...
                    const serverHost = worldname + hubUrl.replace(/^[^\.]+/, ''); // XXX this needs to be core-relative
                    ...
                    console.log('started server ', JSON.stringify(serverSpec, null, 2), JSON.stringify({pid}, null, 2));
                ...
                _error(new Error('No ports available in range ' + JSON.stringify(SERVER_START_PORT)));
              }
            })
            .catch(err => {
              _error(err);

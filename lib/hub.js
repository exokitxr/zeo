...
const listen = (a, config, {key, userDb}) => {
  ...
  const {hostname: homeHostname} = _parseUrl(homeUrl);
  ...
  const _listenServers = () => new Promise((accept, reject) => {
    class Server {
      constructor(worldname, url, protocol, serverPort, homePort, users, running, address, timestamp) {
    ...
    a.app.get('/hub/servers/server.json', (req, res, next) => {
      res.json({
        type: 'hub',
        url: null,
      });
    });
    a.app.get('/hub/servers/servers.json', (req, res, next) => {
      res.json({
        servers: _getServers(),
      });
    });
    a.app.post('/hub/servers/announce', bodyParserJson, (req, res, next) => {
      const {body: j} = req;

      const _isValidProtocol = s => /^https?$/.test(s);
      const _isValidHost = s => {
        const match = s.match(/^(.+?)\.(.+?)$/);
        return Boolean(match) && (match[2] === homeHostname);
      };
      ...
      if (
        ...
        typeof j.serverPort === 'number' &&
        typeof j.homePort === 'number' &&
        ...
        if (address) {
          const {worldname, protocol, serverPort, homePort, users} = j;
          const url = protocol + '://' + address + ':' + homePort + '/s/' + worldname;
          const running = true;
          const timestamp = Date.now();

          const server = new Server(worldname, url, protocol, serverPort, homePort, users, running, address, timestamp);
          serversCache.set(url, server);

          res.send();

          console.log('server announce', JSON.stringify([worldname, address, protocol, serverPort, homePort, url]));
        } else {
          res.status(400);
          res.send();
        }

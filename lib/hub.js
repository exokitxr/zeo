...
const fs = require('fs');
const stream = require('stream'); // XXX delete unnecessary libs here
    ...
    const _listenServersRequest = () => {
        ...
        if (server) {
          ...
          fs.lstat(serverIconPath, err => {
            if (!err) {
              ...
              const rs = fs.createReadStream(serverIconPath);
              rs.pipe(res);
            } else {
              res.type('image/png');
      ...
      a.app.get(/^\/servers\/img\/cubemap\/(.+?)\/(top|bottom|left|right|front|back)\.png$/, _filterIsHubHostname, (req, res, next) => {
        ...
        if (server) {
          ...
          res.type('image/png'); // XXX need to handle annoncements from the server here
          res.send(transparentImgBuffer);

          /* imageLib.requestHubImages(worldname)
            .then(({
              cubeMapImgs,
            }) => {

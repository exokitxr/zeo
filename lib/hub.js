      ...
      const _listenStripe = () => new Promise((accept, reject) => {
        ...
        a.app.post('/hub/userState', bodyParser.json(), (req, res, next) => {
          ...
          if (
            ...
            typeof j.state === 'object' && j.state !== null && typeof j.state.world === 'string' && Array.isArray(j.state.matrix) && j.state.matrix.length === 16
          ) {
            const {token, state: {world, matrix}} = j;

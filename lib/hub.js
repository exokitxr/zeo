...
const listen = (a, config, {key, userDb}) => {
  ...
  const _listenServers = () => new Promise((accept, reject) => {
    class Server {
      constructor(worldname, url, serverHost, serverPort, users, running, address, timestamp) {
    ...
    a.app.get('/servers/server.json', _filterIsHubHostname, (req, res, next) => {
      ...
      if (origin) {
        const {hostname: originHostname} = _parseUrl(origin);

        if (originHostname === homeHostname) {
    ...
    a.app.post('/servers/announce', _filterIsHubHostname, bodyParserJson, (req, res, next) => {
      ...
      if (
        ...
        typeof j.serverHost === 'string' && _isValidServerHost(j.serverHost) &&
        ...
        if (address) {
          const {worldname, serverHost, serverPort, users} = j;
          const url = serverHost + ':' + serverPort;
          const running = true;
          const timestamp = Date.now();

          const server = new Server(worldname, url, serverHost, serverPort, users, running, address, timestamp);
          serversCache.set(url, server);

          res.send();

          console.log('server announce', JSON.stringify([worldname, serverHost, serverPort]));
        } else {
          res.status(400);
          res.send();
        }

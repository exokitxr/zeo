          ...
          Promise.all([
            ...
            .then(([
              ...
              const validProcesses = processes.filter(process => {
                ...
                return processIndexFilePath === localIndexFilePath && worldname && serverHost && port;
              });
              const serverSpecs = files.map(serverName => {
                const process = validProcesses.find(process => {
                  const {cmdline} = process;
                  const args = cmdline.slice(1);
                  const serverHost = _findArg(args, 'serverHost');
              ...
              return Promise.all(serverSpecs.map(serverSpec => {
                const {worldname, url, running} = serverSpec;

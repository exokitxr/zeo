  ...
  const _listenServers = () => new Promise((accept, reject) => {
    ...
    a.app.post('/servers/\announce', _filterIsHubHostname, bodyParserJson, (req, res, next) => {
      ...
      if (
        ...
        if (address) {
          const {worldname, serverHost, port, users} = j;
          const url = serverHost + ':' + port;
          const running = true;
          const timestamp = Date.now();

          const server = new Server(worldname, url, serverHost, port, users, running, address, timestamp);
          serversCache.set(url, server);

          res.send();

          console.log('server announce', JSON.stringify([worldname, serverHost, port]));
        } else {
          res.status(400);
          res.send();
        }

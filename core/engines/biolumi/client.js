...
class Biolumi {
  ...
  mount() {
    ...
    const _requestImageBitmap = src =>
      _requestImage(src).then(img =>
    ...
    return Promise.all([
      ...
      (
        [[three, geometryUtils], transparentImg, uiWorker, uiTimer, rasterizer]
      ) => {
        if (live) {
          ...
          if (rasterizer.type === 'internal') {
            console.warn(
              'warning: Server is using *slow* local rendering; VR clients will experience hitching. To fix this, contact the server admin.'
            );
          }
            ...
            constructor(
              ...
              const mesh = (() => {
                ...
                const material = (() => {
                  ...
                  shaderUniforms.backgroundColor.value = Float32Array.from(
                    color
                  );
            ...
            update() {
              ...
              const _requestLayer = () => {
                ...
                if (type === 'html') {
                  ...
                  console.warn(
                    'illegal layer spec type:' + JSON.stringify(type)
                  );
          ...
          class UiTracker {
            ...
            update({ pose, sides, controllerMeshes }) {
              ...
              const { meshes, open } = this;
              if (open && meshes.length > 0) {
                const { gamepads } = pose;
                const { hoverStates, dotMeshes, boxMeshes } = this;
                ...
                for (let s = 0; s < SIDES.length; s++) {
                  ...
                  if (sides.includes(side)) {
                    const controllerLine = localLine.set(
                      ...
                      localVector.copy(gamepad.worldPosition).add(
                        localVector2
                          .copy(forwardVector)
                    ...
                    for (let i = 0; i < meshes.length; i++) {
                      ...
                      if (_isWorldVisible(mesh)) {
                        ...
                        const uiPlane = localPlane.setFromNormalAndCoplanarPoint(
                          normal,
                          localVector3.setFromMatrixPosition(mesh.matrixWorld)
                        );
                        const intersectionPoint = uiPlane.intersectLine(
                          controllerLine,
                          localVector3
                        );
                        ...
                        if (intersectionPoint) {
                          ...
                          const worldPosition = localVector4.setFromMatrixPosition(
                            mesh.matrixWorld
                          );
                          const worldRotation = localQuaternion.setFromRotationMatrix(
                            mesh.matrixWorld
                          );
                          ...
                          const yAxis = localPlane2.setFromNormalAndCoplanarPoint(
                            ...
                            localVector6
                              ...
                              .sub(
                                localVector7
                                  .set(worldWidth / 2, 0, 0)
                                  .applyQuaternion(worldRotation)
                              )
                          );
                          const x =
                            yAxis.distanceToPoint(intersectionPoint) /
                            worldWidth *
                            width;
                          const xAxis = localPlane3.setFromNormalAndCoplanarPoint(
                            ...
                            localVector6
                              ...
                              .add(
                                localVector7
                                  .set(0, worldHeight / 2, 0)
                                  .applyQuaternion(worldRotation)
                              )
                          );
                          const y =
                            xAxis.distanceToPoint(intersectionPoint) /
                            worldHeight *
                            height;
                          ...
                          if (x >= 0 && x < width && y > 0 && y <= height) {
                            ...
                            if (anchor) {
                              ...
                              hoverState.crossValue =
                                (y - anchor.top) / (anchor.bottom - anchor.top);
                              ...
                              boxMesh.position
                                ...
                                .add(
                                  localVector7
                                    .set(
                                      -worldWidth / 2 + anchorMidpoint.x,
                                      worldHeight / 2 - anchorMidpoint.y,
                                      0
                                    )
                                    .applyQuaternion(worldRotation)
                                );
                            ...
                            if (
                              !gamepad.buttons.trigger.pressed &&
                              dotMesh.material.color.getHex() !== RAY_COLOR
                            ) {
                              dotMesh.material.color.setHex(RAY_COLOR);
                            } else if (
                              gamepad.buttons.trigger.pressed &&
                              dotMesh.material.color.getHex() !==
                                RAY_HIGHLIGHT_COLOR
                            ) {
                            ...
                            rayMesh.scale.z = intersectionPoint.distanceTo(
                              localLine.start
                            );
                            rayMesh.updateMatrixWorld();
                            rayMesh.visible = true;
          ...
          const _makeUi = ({ width, height, color = [1, 1, 1, 1] }) =>
            new Ui(width, height, color);
          const _makePage = (
            spec,
            {
              type = null,
          ...
          const _applyStateKeyEvent = (state, e) => {
            ...
            if (_isPrintableKeycode(e.event.keyCode)) {
              ...
              state.inputText =
                inputText.slice(0, inputIndex) +
                _getKeyEventCharacter(e.event) +
                inputText.slice(inputIndex);
              state.inputIndex++;
              ...
              if (inputIndex > 0) {
                state.inputText =
                  inputText.slice(0, inputIndex - 1) +
                  inputText.slice(inputIndex);
                state.inputIndex--;
                state.inputValue = width * state.inputIndex;
          ...
          const _makeMeshPointGetter = ({
            ...
            position.clone().add(
              new THREE.Vector3(-worldWidth / 2, worldHeight / 2, 0)
                .add(
                  new THREE.Vector3(
                    x / width * worldWidth,

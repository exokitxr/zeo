      ...
      .then(([three]) => {
        if (live) {
          ...
          wss.on('connection', c => {
            ...
            if (
              (match = url.match(
                /^\/archae\/multiplayerWs\?id=(.+?)&address=(.+?)&username=(.+?)$/
              ))
            ) {
              if (connections.length < maxUsers) {
                ...
                const remoteAddress = c.upgradeReq.connection.remoteAddress.replace(
                  /^::ffff:/,
                  ''
                );
                console.log('multiplayer connection', {
                  n,
                  address,
                  username,
                  remoteAddress,
                ...
                c.on('message', o => {
                  if (typeof o === 'string') {
                    ...
                    if (type === 'setSkin') {
                      ...
                      console.warn(
                        'multiplayer unknown message type',
                        JSON.stringify(type)
                      );
                    }
                  } else {
                    if (!pendingMessage) {
                      // update
                      const n = protocolUtils.parseUpdateN(
                        o.buffer,
                        o.byteOffset
                      );
                      ...
                      if (status) {
                        ...
                        console.warn(
                          'multiplayer ignoring status for nonexistent user',
                          { n }
                        );
                      }
                    } else {

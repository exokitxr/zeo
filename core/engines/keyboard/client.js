...
const keyboardHighlightImgSrc =
  'data:image/svg+xml;base64,' + btoa(keyboardHighlightImgString);
const dotsImgSrc = 'data:image/svg+xml;base64,' + btoa(dotsImgString);
const dotsHighlightImgSrc =
  'data:image/svg+xml;base64,' + btoa(dotsHighlightImgString);
          ...
          const keyboardMesh = (() => {
            ...
            const planeMesh = (() => {
              ...
              const headerMesh = (() => {
                ...
                const mesh = new THREE.Mesh(geometry, material);
                mesh.position.y =
                  KEYBOARD_WORLD_HEIGHT / 2 + KEYBOARD_HEADER_WORLD_HEIGHT / 2;
                return mesh;
              })();
              mesh.add(headerMesh);
            ...
            keyboardHeaderPage.mesh.position.copy(
              planeMesh.headerMesh.position
            );
            keyboardHeaderPage.mesh.quaternion.copy(
              planeMesh.headerMesh.quaternion
            );
            ...
            const keySpecs = (() => {
              ...
              for (let i = 0; i < keyEls.length; i++) {
                ...
                const imageData = (() => {
                  ...
                  let imageData = keyboardHighlightCanvas.ctx.getImageData(
                    left,
                    top,
                    width,
                    height
                  ...
                  if (key === 'enter') {
                    // special case the enter key; it has a non-rectangular shape
                    const canvas = document.createElement('canvas');
        ...
        input.on('paste', _paste, {
                ...
                SIDES.forEach(side => {
                  ...
                  if (keydown === 'header') {
                    ...
                    if (gamepad) {
                      ...
                      const controllerEnd = controllerPosition
                        .clone()
                        .add(
                          new THREE.Vector3(
                            0,
                ...
                SIDES.forEach(side => {
                  ...
                  const _hide = () => {
                    ...
                    if (oldKey) {
                      ...
                      const oldKeySpec = keySpecs.find(
                        keySpec => keySpec.key === oldKey
                      );
                  ...
                  if (target && target.type === 'keyboard') {
                    ...
                    } = _decomposeObjectMatrixWorld(planeMesh);
                    const xAxis = new THREE.Vector3(1, 0, 0).applyQuaternion(
                      keyboardRotation
                    );
                    const negativeYAxis = new THREE.Vector3(
                      0,
                      -1,
                      0
                    ).applyQuaternion(keyboardRotation);
                    const keyboardTopLeftPoint = keyboardPosition
                      .clone()
                      .add(
                        xAxis.clone().multiplyScalar(-KEYBOARD_WORLD_WIDTH / 2)
                      )
                      .add(
                        negativeYAxis
                          .clone()
                          .multiplyScalar(-KEYBOARD_WORLD_HEIGHT / 2)
                      );

                    const matchingKeySpec = (() => {
                      const intersectionRay = intersectionPoint
                        .clone()
                        .sub(keyboardTopLeftPoint);
                      const ax = intersectionRay
                        .clone()
                        .projectOnVector(xAxis)
                        .dot(xAxis);
                      const ay = intersectionRay
                        .clone()
                        .projectOnVector(negativeYAxis)
                        .dot(negativeYAxis);
                      const { keySpecs } = keyboardMesh;
                      ...
                      if (matchingKeySpecs.length > 0) {
                        ...
                        if (
                          ...
                          const intersectionPointVector = new THREE.Vector2(
                            x,
                            y
                          );
                          ...
                          return keySpecDistanceSpecs.sort(
                    ...
                    if (matchingKeySpec) {
                      ...
                      if (key !== keyMesh.key) {
                        ...
                        keyMesh.position.copy(
                          ...
                          new THREE.Vector3(
                            -KEYBOARD_WORLD_WIDTH / 2,
                            KEYBOARD_WORLD_HEIGHT / 2,
                            0
                          )
                            .add(
                              new THREE.Vector3(1, 0, 0).multiplyScalar(
                                (left + width / 2) / fullWidth * worldWidth
                              )
                            )
                            .add(
                              new THREE.Vector3(0, -1, 0).multiplyScalar(
                                (top + height / 2) / fullHeight * worldHeight
                              )
                            )
                            .add(
                              new THREE.Vector3(
          ...
          class KeyboardApi {
            ...
            focus({
              ...
              const newFocusState = new KeyboardFocusState({
                type,
                inputText,
                inputIndex,
                inputValue,
              ...
              keyboardMesh.position.copy(
                position
                  .clone()
                  .add(
                    new THREE.Vector3(0, -0.6, -0.4).applyQuaternion(rotation)
                  )
              );

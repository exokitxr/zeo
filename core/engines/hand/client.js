...
class Hand {
  ...
  mount() {
    ...
    return archae
      ...
      .then(
        ([three, input, webvr, rend, multiplayer, jsUtils, networkUtils]) => {
          if (live) {
            ...
            const connection = (() => {
              const connection = new AutoWs(
                _relativeWsUrl('archae/handWs?id=' + localUserId)
              );
            ...
            class Grabbable extends EventEmitter {
              ...
              setLocalTransform(localPosition, localRotation, localScale) {
                if (
                  !this.localPosition.equals(localPosition) ||
                  !this.localRotation.equals(localRotation) ||
                  !this.localScale.equals(localScale)
                ) {

...
class Bootstrap {
  ...
  mount() {
    ...
    const {
      metadata: { site: { url: siteUrl }, server: { url: serverUrl } },
    } = archae;
    ...
    return archae.requestPlugins(['/core/engines/config']).then(([config]) => {
      if (live) {
          ...
          new Promise((accept, reject) => {
            publicIp.v4().then(
              ip => {
                ...
                const req = (siteSpec.protocol === 'http'
                  ? http
                  : https
                ).request(options);
                ...
                req.on('response', res => {
                  ...
                  res.on('end', () => {
                    ...
                    if (statusCode >= 200 && statusCode < 300) {
                      ...
                      const err = new Error(
                        'server announce returned error status code: ' +
                          statusCode
                      );
                      err.code = 'EHTTP';

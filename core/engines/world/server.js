...
class World {
  ...
  mount() {
    ...
    const _requestFile = (p, defaultValue) =>
    ...
    const _requestTagsJson = () =>
      _requestFile(worldTagsJsonPath, DEFAULT_TAGS);
    const _requestFilesJson = () =>
      _requestFile(worldFilesJsonPath, DEFAULT_FILES);
    const _ensureWorldPath = () =>
      new Promise((accept, reject) => {
        const worldPath = path.join(dirname, dataDirectory, 'world');
    ...
    return Promise.all([
      archae.requestPlugins(['/core/engines/multiplayer']),
      _requestTagsJson(),
      _requestFilesJson(),
      _ensureWorldPath(),
    ]).then(([[multiplayer], tagsJson, filesJson, ensureWorldPathResult]) => {
      if (live) {
        const _saveFile = (p, j) =>
          new Promise((accept, reject) => {
        ...
        wss.on('connection', c => {
          ...
          if ((match = url.match(/\/archae\/worldWs\?id=(.+)$/))) {
            ...
            const _sendInit = () => {
              const e = {
                type: 'init',
                args: [_arrayify(tagsJson.tags), _arrayify(usersJson)],
              };
              const es = JSON.stringify(e);
              c.send(es);
            };
            ...
            for (const id in tags) {
              ...
              if (type === 'asset') {
                ...
                const {
                  ...
                  bindOwner: bindOwnerAttribute,
                } = attributes;
                const owner = ownerAttribute ? ownerAttribute.value : null;
                const bindOwner = bindOwnerAttribute
                  ? bindOwnerAttribute.value
                  : null;
          ...
          initTags() {
            ...
            for (const id in tagsJson.tags) {
              ...
              if (tagSpec.type === 'entity') {
                plugins.push(
                  path.isAbsolute(tagSpec.module)
                    ? tagSpec.module
                    : `${tagSpec.module}@${tagSpec.version}`
                );
              }
            }
...
const _getAddress = privateKey =>
  Uint8Array.from(secp256k1.keyFromPrivate(privateKey).getPublic('arr'));

      ...
      .then(
        (
          ...
          if (live) {
            ...
            const RAY_HIGHLIGHT_COLOR = new THREE.Color(RAY_COLOR)
              .multiplyScalar(0.5)
              .getHex();
            ...
            class Player extends EventEmitter {
              ...
              snapshotStatus(status) {
                ...
                this.prevStatusIndex = mod(
                  this.prevStatusIndex + 1,
                  NUM_PREV_STATUSES
                );
              ...
              prevStatus.hmd.worldScale.copy(camera.scale); */
                ...
                prevStatus.gamepads.left.worldPosition.copy(
                  gamepads.left.worldPosition
                );
                prevStatus.gamepads.left.worldRotation.copy(
                  gamepads.left.worldRotation
                );
                prevStatus.gamepads.left.worldScale.copy(
                  gamepads.left.worldScale
                );

                prevStatus.gamepads.right.position.copy(
                  gamepads.right.position
                );
                prevStatus.gamepads.right.rotation.copy(
                  gamepads.right.rotation
                );
                prevStatus.gamepads.right.scale.copy(gamepads.right.scale);
                prevStatus.gamepads.right.worldPosition.copy(
                  gamepads.right.worldPosition
                );
                prevStatus.gamepads.right.worldRotation.copy(
                  gamepads.right.worldRotation
                );
                prevStatus.gamepads.right.worldScale.copy(
                  gamepads.right.worldScale
                );
              ...
              getControllerLinearVelocity(side) {
                ...
                const firstStatus =
                  prevStatuses[mod(prevStatusIndex + 1, NUM_PREV_STATUSES)];

                const positionDiff = lastStatus.gamepads[side].worldPosition
                  .clone()
                  .sub(firstStatus.gamepads[side].worldPosition);
                const timeDiff = lastStatus.timestamp - firstStatus.timestamp;
                return timeDiff > 0
                  ? positionDiff.divideScalar(timeDiff / 1000)
                  : zeroVector;
              }
              ...
              getControllerAngularVelocity(side) {
                ...
                const quaternionDiff = lastStatus.gamepads[side].worldRotation
                  .clone()
                  .multiply(
                    firstStatus.gamepads[side].worldRotation.clone().inverse()
                  );
                const angleDiff = (() => {
                  const x =
                    quaternionDiff.x /
                    Math.sqrt(1 - quaternionDiff.w * quaternionDiff.w);
                  const y =
                    quaternionDiff.y /
                    Math.sqrt(1 - quaternionDiff.w * quaternionDiff.w);
                  const z =
            ...
            const _makePlayerPlaceholderMesh = () => {
              ...
              object.update = (hmdStatus, gamepadsStatus) => {
                ...
                const _updateControllerMeshes = () => {
                  _updateControllerMesh(
                    controllerMeshes.left,
                    gamepadsStatus.left
                  );
                  _updateControllerMesh(
                    controllerMeshes.right,
                    gamepadsStatus.right
                  );
                };
                const _updateControllerMesh = (
                  controllerMesh,
                  gamepadStatus
                ) => {
                  ...
                  controllerMesh.worldPosition.copy(
                    gamepadStatus.worldPosition
                  );
                  controllerMesh.worldRotation.copy(
                  ...
                  if (
                    !buttons.trigger.pressed &&
                    controllerMesh.rayMesh.material.color.getHex() !== RAY_COLOR
                  ) {
                    controllerMesh.rayMesh.material.color.setHex(RAY_COLOR);
                  } else if (
                    buttons.trigger.pressed &&
                    controllerMesh.rayMesh.material.color.getHex() !==
                      RAY_HIGHLIGHT_COLOR
                  ) {
                    controllerMesh.rayMesh.material.color.setHex(
                      RAY_HIGHLIGHT_COLOR
                    );
                  }
                };
            ...
            rend.registerAuxObject(
              'controllerMeshes',
              playerPlaceholderMesh.controllerMeshes
            );
              ...
              const circleMesh = (() => {
                const geometry = (() => {
                  const geometry = new THREE.CylinderBufferGeometry(
                    ...
                    .applyMatrix(
                      new THREE.Matrix4().makeRotationZ(Math.PI / 8)
                    );

                  const _almostZero = v => Math.abs(v) < 0.001;

                  const { array: positions } = geometry.getAttribute(
                    'position'
                  );
                  ...
                  for (let i = 0; i < numPositions; i++) {
                    ...
                    if (
                      _almostZero(positions[baseIndex + 0]) &&
                      _almostZero(positions[baseIndex + 1])
                    ) {
                ...
                const notchMesh = (() => {
                  const geometry = new THREE.SphereBufferGeometry(0.005, 5, 4)
                    ...
                    .applyMatrix(
                      new THREE.Matrix4().makeTranslation(0, 0, 0.003)
                    );
                  const material = solidMaterial;
              ...
              hudMesh.update = () => {
                ...
                if (vrMode === 'keyboard' && mode !== null && keys !== null) {
                  ...
                  if (axis) {
                    ...
                    const gamepad =
                      gamepadStatus[mode === 'center' ? 'left' : mode];
                    const { axes } = gamepad;
                    notchMesh.position.set(
                      axes[0] * 0.043,
                      axes[1] * 0.043,
                      (1 - new THREE.Vector2(axes[0], axes[1]).length()) *
                        -0.005
                    );
            ...
            const _update = () => {
              ...
              if (playerSkinMesh) {
                ...
                localSkinStatus.gamepads.left.position =
                  gamepadsStatus.left.worldPosition;
                localSkinStatus.gamepads.left.rotation =
                  gamepadsStatus.left.worldRotation;
                localSkinStatus.gamepads.right.position =
                  gamepadsStatus.right.worldPosition;
                localSkinStatus.gamepads.right.rotation =
                  gamepadsStatus.right.worldRotation;

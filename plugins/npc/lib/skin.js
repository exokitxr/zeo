...
const SKIN_SHADER = {
  ...
  vertexShader: [
    ...
    "  gl_Position = projectionMatrix * modelViewMatrix * vec4(position.x, position.y - dy.y + (dy.y*cos(theta2) - dy.z*sin(theta2)), position.z + dy.z + (dy.z*cos(theta2) + dy.y*sin(theta2)), 1.0);",
    "  vUv = uv;",
    "}"
  ].join("\n"),
  fragmentShader: [

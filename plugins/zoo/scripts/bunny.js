...
const dyCutoffBox = new THREE.Box3().setFromCenterAndSize(
  ...
  new THREE.Vector3(2, 3.45, 3.6).multiplyScalar(globalScale)
);
const splitX = 0;
const splitZ = -0.5 * globalScale;
const dhCutoffBox = new THREE.Box3().setFromCenterAndSize(
  new THREE.Vector3(0, 3.5, -1.2).multiplyScalar(globalScale),
  new THREE.Vector3(2, 3.5, 2.2).multiplyScalar(globalScale)
);

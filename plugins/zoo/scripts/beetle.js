...
const _makeCutoffBox = i => new THREE.Box3().setFromCenterAndSize(
  new THREE.Vector3(1.875 * (i === 0 ? -1 : 1), 0, 0).multiplyScalar(globalScale),
  new THREE.Vector3(1.875, 1.5, 7.5).multiplyScalar(globalScale)
);
const dyCutoffBoxes = [
  _makeCutoffBox(0),
  _makeCutoffBox(1),
...
const splitX = 0;
const splitZ = 1 * globalScale;
const dhCutoffBox = new THREE.Box3().setFromCenterAndSize(
  new THREE.Vector3(0, 0.75, -1.6875).multiplyScalar(globalScale),
  new THREE.Vector3(1.5, 1.2, 1.875).multiplyScalar(globalScale)
);

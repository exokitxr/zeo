...
const dyCutoffBox = new THREE.Box3().setFromCenterAndSize(
  new THREE.Vector3(0, 0, -1.5).multiplyScalar(globalScale),
  new THREE.Vector3(16, 30, 25).multiplyScalar(globalScale)
);
const splitX = 0;
const splitZ = 0;
const dhCutoffBoxes = [
  new THREE.Box3().setFromCenterAndSize(
    new THREE.Vector3(0, 25, -20).multiplyScalar(globalScale),
    new THREE.Vector3(20, 17, 16.5).multiplyScalar(globalScale)
  ),
  new THREE.Box3().setFromCenterAndSize(
    new THREE.Vector3(0, 14, -25).multiplyScalar(globalScale),
    new THREE.Vector3(14, 24, 21).multiplyScalar(globalScale)
  ),
];
...
console.warn('min y', min.y);
  ...
  for (let i = 0; i < positions.length / 3; i++) {
    ...
    if (dhCutoffBoxes.some(cutoffBox => cutoffBox.containsPoint(v))) {
      const k = v.toArray().join(':');
      dhVertices[k] = true;
    }
  }

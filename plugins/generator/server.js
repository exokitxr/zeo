...
const { BLOCK_BUFFER_SIZE, GEOMETRY_BUFFER_SIZE } = zeode;
...
class Generator {
  ...
  mount() {
    ...
    const texturesPngDataPath = path.join(
      dirname,
      dataDirectory,
      'textures.png'
    );
    const texturesJsonDataPath = path.join(
      dirname,
      dataDirectory,
      'textures.json'
    );
    ...
    const _generateChunkTerrain = (chunk, opts) => {
      ...
      protocolUtils.stringifyTerrainData(
        terrainTesselator.generate(chunk.x, chunk.z, opts),
        uint32Buffer.buffer,
    ...
    const _decorateChunkObjectsGeometry = chunk => {
      ...
      protocolUtils.stringifyGeometry(
        objectsTesselator.tesselate(chunk),
    ...
    const _getZeode = () =>
      _readEnsureFile(zeodeDataPath).then(b => {
        const zde = zeode();
        if (b) {
          zde.load(b);
    ...
    const _getTextures = () =>
      Promise.all([
        ...
        _readEnsureFile(texturesJsonDataPath, 'utf8').then(s => {
          if (s !== null && s.length > 0) {
            const { atlas, uvs } = JSON.parse(s);
            return [txtr.fromJson(atlas), uvs];
          } else {
            return [txtr(TEXTURE_SIZE, TEXTURE_SIZE), {}];
          }
        }),
      ]).then(([textureImg, [textureAtlas, textureUvs]]) => ({
        textureImg,
        textureAtlas,
        textureUvs,
      }));
    ...
    return Promise.all([
      _getZeode(),
      _getTextures(),
    ]).then(([zde, { textureImg, textureAtlas, textureUvs }]) => {
      ...
      const _decorateChunkLightsRange = (
        ...
        _ensureChunksHardRange(chunk.x, chunk.z).then(() => {
      ...
      const _requestChunkWithLights = chunk =>
        chunk[lightsRenderedSymbol]
          ? Promise.resolve(chunk)
          : _decorateChunkLights(chunk);
        ...
        if (chunk[lightmapsSymbol]) {
          ...
          const _getTerrainLightmaps = () => {
            ...
            const {
              positions,
              staticHeightfield,
            } = protocolUtils.parseTerrainData(
            ...
            const skyLightmaps = new Uint8Array(
              lightmapsBuffer,
              0,
              numLightmaps
            );
            const torchLightmaps = new Uint8Array(
              lightmapsBuffer,
              numLightmaps,
              numLightmaps
            );
          ...
          const _getObjectLightmaps = () => {
            const geometryBuffer = chunk.getGeometryBuffer();
            const { positions } = protocolUtils.parseGeometry(
              geometryBuffer.buffer,
              geometryBuffer.byteOffset
            );
      ...
      const _writeFileData = (p, data, byteOffset) =>
        new Promise((accept, reject) => {
      ...
      const _saveChunks = _debounce(next => {
        ...
        zde.save((byteOffset, data) => {
          promises.push(
            _writeFileData(
              zeodeDataPath,
              new Buffer(data.buffer, data.byteOffset, data.byteLength),
              byteOffset
            )
          );
        });
      ...
      class Generator {
        ...
        registerGeometry(name, geometry) {
          if (!geometry.getAttribute('ssao')) {
            const ssaos = new Uint8Array(
              geometry.getAttribute('position').array.length / 3
            );
          ...
          if (!geometry.getAttribute('frame')) {
            ...
            geometry.addAttribute(
              'frame',
              new THREE.BufferAttribute(frames, 3)
            );
          }
          ...
          geometriesOffset = protocolUtils.stringifyTemplate(
            ...
            geometriesBuffer.buffer,
            geometriesBuffer.byteOffset + geometriesOffset
          )[1];
        ...
        registerBlock(name, blockSpec) {
          ...
          for (let d = 0; d < 6; d++) {
            faceUvs.set(
              Float32Array.from(blockSpec.uvs[d]),
              index * 6 * 4 + d * 4
            );
          }
        }
          ...
          if (zde.chunks.length > 0) {
            ...
            for (let i = 0; i < zde.chunks.length; i++) {
              ...
              if (_generateChunkObjectsWithGenerator(chunk, generator)) {
                promises.push(
                  _decorateChunkObjectsGeometry(chunk).then(() => {
                    chunk.dirty = true;
                  })
                );
              }
      ...
      function serveObjectsObjectizeJs(req, res, next) {
        ...
        fs
          .createReadStream(path.join(vxlPath, 'bin', 'objectize.js'))
          .pipe(res);
      }
      ...
      function serveObjectsObjectizeWasm(req, res, next) {
        ...
        fs
          .createReadStream(path.join(vxlPath, 'bin', 'objectize.wasm'))
          .pipe(res);
      }
      ...
      function serveObjectsTemplates(req, res, next) {
        res.write(
          new Buffer(
            geometriesBuffer.buffer,
            geometriesBuffer.byteOffset,
            geometriesBuffer.byteLength
          )
        );
        res.write(
          new Buffer(
            geometryTypes.buffer,
            geometryTypes.byteOffset,
            geometryTypes.byteLength
          )
        );
        res.write(
          new Buffer(
            blockTypes.buffer,
            blockTypes.byteOffset,
            blockTypes.byteLength
          )
        );
        res.write(
          new Buffer(
            transparentVoxels.buffer,
            transparentVoxels.byteOffset,
            transparentVoxels.byteLength
          )
        );
        res.write(
          new Buffer(
            translucentVoxels.buffer,
            translucentVoxels.byteOffset,
            translucentVoxels.byteLength
          )
        );
        res.write(
          new Buffer(faceUvs.buffer, faceUvs.byteOffset, faceUvs.byteLength)
        );
        res.write(
          new Buffer(lights.buffer, lights.byteOffset, lights.byteLength)
        );
        res.end();
      }
          ...
          _requestChunk(x, z)
            .then(chunk => {
              ...
              res.write(
                new Buffer(
                  objectBuffer.buffer,
                  objectBuffer.byteOffset,
                  objectBuffer.byteLength
                )
              );
              ...
              const [
                arrayBuffer,
                byteOffset,
              ] = protocolUtils.stringifyDecorations(
                chunk[lightmapsSymbol],
                chunk[blockfieldSymbol]
              );
        ...
        if (url === '/archae/generatorWs') {
          ...
          c.on('message', msg => {
            ...
            if (method === 'mutateVoxel') {
                ...
                if (!seenIndex[index]) {
                  ...
                  if (chunk) {
                    ...
                    regeneratePromises.push(
                      generatorElement.requestRelight(x, y, z)
                    );
                  }
              ...
              Promise.all(regeneratePromises).then(regeneratedChunks => {
                c.send(
                  JSON.stringify({
                    type: 'response',
                    id,
              ...
              if (chunk) {
                ...
                _decorateChunkObjectsGeometry(chunk)
                  .then(() => {
                    ...
                    return generatorElement.requestRelight(
                      Math.floor(positions[0]),
                      Math.floor(positions[1]),
                      Math.floor(positions[2])
                    );
                  })
                  .then(() => {
                    c.send(
              ...
              console.warn(
                'objects server got unknown method:',
